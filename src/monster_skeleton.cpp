		...
		if ( myStats != nullptr )
		{
			...
			else
			{
				...
				if ( myStats->helmet == nullptr && myStats->EDITOR_ITEMS[ITEM_SLOT_HELM] == 1 )
				{
					switch ( local_rng.rand() % 10 )
					{
						//case 0: myStats->helmet = newItem(HAT_PIRATE, DECREPIT, -1 + local_rng.rand() % 2, 1, local_rng.rand(), false, nullptr);
							//break;
						case 0:
						case 1:
						case 2:
			...
			if (isDefaultStats)
			{
				myStats->MAXHP += HP_MOD * avglvl;
				myStats->HP += HP_MOD * avglvl;
				myStats->STR += avglvl;
			}
