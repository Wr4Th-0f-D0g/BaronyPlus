...
namespace ConsoleCommands {
  ...
  messagePlayer(clientnum, MESSAGE_MISC, "command '%s' not found", cmd);
	...
	static ConsoleCommand ccmd_setloop("/setloop", "set value of loop variable", []CCMD{
		if (!(svFlags & SV_FLAG_CHEATS))
		{
			messagePlayer(clientnum, MESSAGE_MISC, Language::get(277));
			return;
		...
		if (svFlags & SV_FLAG_ENDLESS)
		{
			loop = atoi(argv[1]);
			messagePlayer(clientnum, MESSAGE_MISC, Language::get(9801), loop);
		}
		});
	...
	static ConsoleCommand ccmd_numitems("/numitems", "display number of items in the level", []CCMD{

			Sint32 i = 0;
			for (node_t* node = map.entities->first; node != nullptr; node = node->next)
			{
				Entity* item = (Entity*)node->element;
				if (item && (item->behavior == &actItem))
				{
					++i;
				}
			}
		messagePlayer(clientnum, MESSAGE_MISC, Language::get(9800), i);
		});
	...
	static ConsoleCommand ccmd_splitscreen("/splitscreen", "enable splitscreen mode (cheat)", []CCMD{
		...
		for (int i = 0; i < MAXPLAYERS; ++i)
		{
			...
			if (i > 0)
			{
				...
				stats[i]->clearStats();
				client_classes[i] = local_rng.rand() % (CLASS_ASCETIC + 1);//NUMCLASSES;
				...
				if (enabledDLCPack1 || enabledDLCPack2)
				{
					...
					if (stats[i]->playerRace == RACE_HUMAN)
					{
						...
						if (!enabledDLCPack1)
						{
							while (client_classes[i] == CLASS_CONJURER || client_classes[i] == CLASS_ACCURSED
								...
								|| client_classes[i] == CLASS_APOTHECARY || client_classes[i] == CLASS_NECROKNIGHT)
							{
								client_classes[i] = local_rng.rand() % (NUMCLASSES);
							}
						}
						else if (!enabledDLCPack2)
						{
							while (client_classes[i] == CLASS_HUNTER || client_classes[i] == CLASS_SHAMAN
								...
								|| client_classes[i] == CLASS_EXTERMINATOR || client_classes[i] == CLASS_LOREMASTER)
							{
								client_classes[i] = local_rng.rand() % (NUMCLASSES);
							}
						}
					...
					{
						client_classes[i] = local_rng.rand() % (CLASS_ASCETIC + 2);
						if (client_classes[i] > CLASS_ASCETIC)
						{
							client_classes[i] = CLASS_ASCETIC + stats[i]->playerRace; // monster specific classes.
						}
...
#else
			client_classes[clientnum] = std::min((int)CLASS_ASCETIC, std::max((int)CLASS_BARBARIAN, atoi(argv[1])));
#endif
		}
		else
		{
			client_classes[clientnum] = local_rng.rand() % (CLASS_ASCETIC + 1);
		}
		});
