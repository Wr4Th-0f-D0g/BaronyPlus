...
void loadAllScores(const std::string& scoresfilename)
{
	...
	for ( int i = 0; i < numscores; i++ )
	{
		...
		for ( int c = 0; c < numitems; c++ )
		{
			...
			newInventoryItem(type, status, stage, beatitude, count, appearance, identified, &score->stats->inventory);
		}
		fp->read(&c, sizeof(Uint32), 1);
	...
	if (players[statsPlayer]->isLocalPlayer()) {
		...
		for (auto& item : p.npc_equipment) {
			...
			if (find != slots.end()) {
				...
				Sint16 stage = item.second.stage;
