...
Sint32 statGetCON(Stat* entitystats, Entity* my)
{
	...
	{
		if (entitystats->helmet->type == NECRO_HELM)
		{
			if (entitystats->helmet->beatitude <= 0)
			{
				CON += entitystats->helmet->beatitude; // lowers CON based on curse value
				CON -= (cursedItemIsBuff ? 1 : 0); // reduces CON even further if demon race
			}
		}
	}
	if ( entitystats->EFFECTS[EFF_SHRINE_RED_BUFF] )
...
Sint32 statGetINT(Stat* entitystats, Entity* my)
{
	...
	if ( entitystats->helmet != nullptr )
	{
		...
		else if (entitystats->helmet->type == NECRO_HELM)
		{
			INT += 4; // increases INT regardless if you're a demon race or not
		}
	}
				...
				else
				{
					...
					switch ( myStats->weapon->type )
					{
						...
						case SPELLBOOK_FEAR:
							...
							break;
						case SPELLBOOK_COMPEL:
							castSpell(uid, &spell_compel, true, false);
							break;
...
char const * playerClassLangEntry(int classnum, int playernum)
{
	...
	else if ( classnum >= CLASS_SEXTON && classnum <= CLASS_ASCETIC )
	{
...
void Entity::setHelmetLimbOffset(Entity* helm)
{
	...
	if ( helm->sprite == items[HAT_PHRYGIAN].index )
	{
		...
		|| helm->sprite == items[PUNISHER_HOOD].index || helm->sprite == items[NECRO_HELM].index)
	{
		switch ( monster )
		{
			case AUTOMATON:
	...
	else if (helm->sprite == items[HAT_PIRATE].index)
	{
		switch (monster)
		{
		case AUTOMATON:
		...
		case SUCCUBUS:
			helm->focalx = limbs[monster][9][0];
			helm->focaly = limbs[monster][9][1] - 4.25;
			helm->focalz = limbs[monster][9][2] + 2.5;
			break;
		case VAMPIRE:
		case SHOPKEEPER:
		...
		case GOATMAN:
			helm->focalx = limbs[monster][9][0];
			helm->focaly = limbs[monster][9][1] - 4.6;
			helm->focalz = limbs[monster][9][2] + 2.75;
			break;
		case INSECTOID:
			helm->focalx = limbs[monster][9][0] + 0.25;
			helm->focaly = limbs[monster][9][1] - 4.25;
			helm->focalz = limbs[monster][9][2] + 2.25;
			break;
		case GOBLIN:
