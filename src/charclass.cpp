...
void initClassStats(const int classnum, void* myStats)
{
	...
	else if (classnum == CLASS_BUCCANEER)
	{
		...
		// skills
		stat->PROFICIENCIES[PRO_SWORD] = 40;
		stat->PROFICIENCIES[PRO_SHIELD] = 25;
		stat->PROFICIENCIES[PRO_RANGED] = 25;
		stat->PROFICIENCIES[PRO_AXE] = 25;
		stat->PROFICIENCIES[PRO_SWIMMING] = 25;
	...
	else if (classnum == CLASS_ASCETIC)
	{
		...
		stat->PROFICIENCIES[PRO_LEADERSHIP] = 10;
		stat->PROFICIENCIES[PRO_SWIMMING] = 20;
	}
	// start DLC
	...
	else if (classnum == CLASS_APOTHECARY)
	{
		// attributes
		stat->STR -= 2;
		stat->CON += 1;
		...
		// skills
		stat->PROFICIENCIES[PRO_ALCHEMY] = 60;
		stat->PROFICIENCIES[PRO_TRADING] = 25;
...
void initClass(const int player)
{
	...
	else if (client_classes[player] == CLASS_BUCCANEER)
	{
		...
		// pirate hat
		item = newItem(HAT_PIRATE, WORN, 0, 1, 0, true, nullptr);
		if (isLocalPlayer)
		{
			item2 = itemPickup(player, item);
			useItem(item2, player);
	...
	else if (client_classes[player] == CLASS_BATTLEMAGE)
	{
		...
		// bronze shield
		item = newItem(BRONZE_SHIELD, SERVICABLE, 0, 1, 0, true, nullptr);
		if (isLocalPlayer)
		{
			item2 = itemPickup(player, item);
			useItem(item2, player);
		...
		// iron boots
		item = newItem(IRON_BOOTS, SERVICABLE, 0, 1, 0, true, nullptr);
		if (isLocalPlayer)
		{
			item2 = itemPickup(player, item);
			useItem(item2, player);
		...
		if (isLocalPlayer)
		{
			...
			// book of compel
			item = newItem(SPELLBOOK_COMPEL, WORN, 0, 1, 5, true, nullptr);
			item2 = itemPickup(player, item);
			hotbar[8].item = item2->uid;
			free(item);
	...
	else if (client_classes[player] == CLASS_NECROKNIGHT)
	{
		...
		// helm
		item = newItem(NECRO_HELM, WORN, -1, 1, 0, true, nullptr);
		if (isLocalPlayer)
		{
			item2 = itemPickup(player, item);
			useItem(item2, player);
		...
		// black cloak
		item = newItem(CLOAK_BLACK, WORN, 0, 1, 0, true, nullptr);
		if (isLocalPlayer)
		{
			item2 = itemPickup(player, item);
			useItem(item2, player);
		...
		{
			// spellbook of cold
			item = newItem(SPELLBOOK_COLD, WORN, 0, 1, 8, true, nullptr);
			item2 = itemPickup(player, item);
			hotbar[9].item = item2->uid;
			free(item);
	...
	else if (client_classes[player] == CLASS_APOTHECARY)
	{
		initClassStats(client_classes[player], stats[player]);
		...
		// tunic
		item = newItem(TUNIC, SERVICABLE, 0, 1, 0, true, nullptr);
		if (isLocalPlayer)
		{
			item2 = itemPickup(player, item);
			useItem(item2, player);
		...
		{
			// acid
			item = newItem(POTION_ACID, EXCELLENT, 0, 4, 0, true, nullptr);
			item2 = itemPickup(player, item);
			hotbar[2].item = item2->uid;
			free(item);

			// healing
			item = newItem(POTION_HEALING, EXCELLENT, 0, 3, 0, true, nullptr);
			item2 = itemPickup(player, item);
			hotbar[8].item = item2->uid;
			free(item);
			...
			// confusion
			item = newItem(POTION_CONFUSION, EXCELLENT, 0, 2, 0, true, nullptr);
			item2 = itemPickup(player, item);
			free(item);
			...
			// apple
			item = newItem(FOOD_APPLE, EXCELLENT, 0, 1, 0, true, nullptr);
			item2 = itemPickup(player, item);
			free(item);
			...
			free(item);
			/*
			item = newItem(READABLE_BOOK, SERVICABLE, 0, 1, getBook("Adventurer's Guide to Alchemy"), true, nullptr);
			item2 = itemPickup(player, item);
			free(item);
			*/
		}
	...
	else if (client_classes[player] == CLASS_LOREMASTER)
	{
		...
		{
			// staff of light
			item = newItem(MAGICSTAFF_LIGHT, EXCELLENT, 0, 1, 0, true, nullptr);
			item2 = itemPickup(player, item);
			free(item);
			...
			item = newItem(SCROLL_MAGICMAPPING, SERVICABLE, 0, 1, 0, true, nullptr);
			item2 = itemPickup(player, item);
			free(item);
			...
			// levitate potion
			item = newItem(POTION_LEVITATION, EXCELLENT, 0, 1, 0, true, nullptr);
			item2 = itemPickup(player, item);
			free(item);
		}
	}
	...
	if ( stats[player]->appearance == 0 
		&& client_classes[player] <= CLASS_ASCETIC 
	...
	if ( isLocalPlayer )
	{
		...
		else if ( client_classes[player] == CLASS_PUNISHER )
		{
			...
			addSpell(SPELL_DEMON_ILLUSION, player, true);
				  ...
				  client_classes[player] == CLASS_NECROKNIGHT )
		{
