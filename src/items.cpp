...
Item* newInventoryItem(ItemType type, Status status, Sint16 stage, Sint16 beatitude, Sint16 count, Uint32 appearance, bool identified, list_t* inventory)
{
	Item* item;
...
Sint32 Item::weaponGetAttack(const Stat* const wielder) const
{
	...
	{
		attack += std::max(base + 2, static_cast<int>(base * 1.5));
	}
	...
	{
		attack += std::max(base + 1, static_cast<int>(base * 1.25));
	}
	...
	{
		attack += std::max(base + 3, static_cast<int>(base * 1.75));
	}
	else if ( type == LONGBOW )
	{
		attack += std::max(base + 6, static_cast<int>(base * 2.5));
	}
	else if ( type == HEAVY_CROSSBOW )
	{
		attack += std::max(base + 12, static_cast<int>(base * 4));
	}
	else if ( type == COMPOUND_BOW )
	{
		attack += std::max(base + 5, static_cast<int>(base * 2.25));
	}
	...
	{
		attack += std::max(base + 4, static_cast<int>(base * 2));
	}
	else if ( type == BOOMERANG )
	{
		return (attack + std::max(0, (status - 1) * art));
	}
	...
	{
		attack += std::max(base + 8, static_cast<int>(base * 3));
	}
	else if ( type == TOOL_WHIP )
	{
		attack += std::max(base - 2, static_cast<int>(base * 0.5));
	}
	else if ( type == QUIVER_SILVER )
	{
		return attack + std::max(base -2 , static_cast<int>(base * 0.5));
	}
	...
	{
		return attack + std::max(base + 2, static_cast<int>(base * 1.5));
	}
...
Sint32 Item::armorGetAC(const Stat* const wielder) const
{
	...
	{
		armor += base + std::max(1, static_cast<int>(stage * 0.33));
	}
	else if ( type == STEEL_HELM )
	{
		armor += base + std::max(2, static_cast<int>(stage * 0.66));
	}
	...
	{
		armor += base + std::max(3, static_cast<int>(stage));
	}
	...
	{
		armor += base + std::max(4, static_cast<int>(stage * 1.33));
	}
	...
	{
		armor += (3 + (status - 1)) * std::max(1, static_cast<int>(stage * 0.33)); // 2-6
	}
	else if ( type == ARTIFACT_HELM)
	{
		armor += (1 + (status - 1)) * std::max(1, static_cast<int>(stage * 0.33)); // 1-4
	}
