...
Sint32 getStatAttributeBonusFromItem(const int player, Item& item, std::string& attribute)
{
	...
	if (item.type == NECRO_HELM)
	{
		if (attribute == "CON" && cursedItemIsBuff)
		{
			stat += item.beatitude;
		}
...
void ItemTooltips_t::formatItemDetails(const int player, std::string tooltipType, Item& item, std::string& str, std::string detailTag)
{
		...
		|| tooltipType.find("tooltip_ring") != std::string::npos )
	{
		...
		{
			snprintf(buf, sizeof(buf), str.c_str(), (ref->armorGetAC() - item.beatitude));
		...
		else if (detailTag.compare("STR_stat_bonus") == 0)
		{
			int baseStatBonus = 0;
			int beatitudeStatBonus = 0;
		...
		else if (detailTag.compare("DEX_stat_bonus") == 0)
		{
			int baseStatBonus = 0;
			int beatitudeStatBonus = 0;
		...
		else if (detailTag.compare("CON_stat_bonus") == 0)
		{
			int baseStatBonus = 0;
			int beatitudeStatBonus = 0;

			//bool cursedItemIsBuff = shouldInvertEquipmentBeatitude(stats[player]); // for NECRO_HELM
			...
			if (items[item.type].hasAttribute(stat))
			{
				baseStatBonus = items[item.type].attributes[stat];
				beatitudeStatBonus = getStatAttributeBonusFromItem(player, item, stat) - baseStatBonus;
				//beatitudeStatBonus -= (item.type == NECRO_HELM && cursedItemIsBuff ? 1 : 0); // subtracts 1 CON in tooltips if wearing necro helm while as demon race

				snprintf(buf, sizeof(buf), str.c_str(), baseStatBonus, stat.c_str(),
					beatitudeStatBonus, stat.c_str(), getItemBeatitudeAdjective(item.beatitude).c_str());
			}
		}
		else if (detailTag.compare("INT_stat_bonus") == 0)
		{
			int baseStatBonus = 0;
			int beatitudeStatBonus = 0;
		...
		else if (detailTag.compare("PER_stat_bonus") == 0)
		{
			int baseStatBonus = 0;
			int beatitudeStatBonus = 0;
		...
		else if (detailTag.compare("CHR_stat_bonus") == 0)
		{
			int baseStatBonus = 0;
			int beatitudeStatBonus = 0;
	...
	else if ( tooltipType.find("tooltip_gem") != std::string::npos )
	{
		...
		{
			snprintf(buf, sizeof(buf), str.c_str(), (ref->weaponGetAttack() - item.beatitude));
		...
		|| tooltipType.find("tooltip_quiver") != std::string::npos )
	{
		...
		if ( detailTag.compare("weapon_base_atk") == 0 )
		{
			...
			{
				snprintf(buf, sizeof(buf), str.c_str(), 1 + (ref->weaponGetAttack() - item.beatitude) - (item.status - 3));
					//items[item.type].hasAttribute("ATK") ? items[item.type].attributes["ATK"] + 1 : 0);
			}
			else
			{
				snprintf(buf, sizeof(buf), str.c_str(), (ref->weaponGetAttack() - item.beatitude) - (item.status - 3));
					//items[item.type].hasAttribute("ATK") ? items[item.type].attributes["ATK"] : 0);
			}
		}
		else if ( detailTag.compare("on_bless_or_curse") == 0 )
