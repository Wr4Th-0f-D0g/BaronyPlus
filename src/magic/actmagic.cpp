...
void actMagicMissile(Entity* my)   //TODO: Verify this function.
{
	...
	if (magic_init)
	{
		...
		if ( multiplayer != CLIENT )
		{
			...
			if ( spell->ID == SPELL_CHARM_MONSTER || spell->ID == SPELL_ACID_SPRAY || spell->ID == SPELL_COMPEL)
			{
			...
			if ( hitFromAbove || (my->actmagicIsVertical != MAGIC_ISVERTICAL_XYZ && dist != sqrt(my->vel_x * my->vel_x + my->vel_y * my->vel_y)) )
			{
				...
				else if (!strcmp(element->element_internal_name, spellElement_compel.element_internal_name))
				{
					if (hit.entity)
					{
						if (hit.entity->behavior == &actMonster || hit.entity->behavior == &actPlayer)
						{
							// compel knockback effect
							Entity* parent = uidToEntity(my->parent);
							real_t pushbackMultiplier = 0.6;// +(0.2 * spellbookDamageBonus);
							if (!hit.entity->isMobile())
							{
						...
						{
							int damage = element->damage;
							damage += 2; // compel deals bonus damage to doors
							damage += (spellbookDamageBonus * damage);
							damage /= (1 + (int)resistance);
							hit.entity->doorHandleDamageMagic(damage, *my, parent);
							my->removeLightField();
						...
						{
							int damage = element->damage;
							damage += 2; // compel deals bonus damage to furniture
							damage += (spellbookDamageBonus * damage);
							damage /= (1 + (int)resistance);
							hit.entity->furnitureHealth -= damage;
							if (parent)
