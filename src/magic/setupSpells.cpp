...
void setupSpells()   ///TODO: Verify this function.
{
	...
	spellElement_force.damage = static_cast<int>(15 * multi);
	...
	spellElement_fire.damage = static_cast<int>(25 * multi);
	...
	spellElement_lightning.damage = static_cast<int>(25 * multi);
	...
	spellElement_cold.damage = static_cast<int>(20 * multi);
	...
	spellElement_magicmissile.damage = static_cast<int>(30 * multi);
	...
	spellElement_bleed.damage = static_cast<int>(30 * multi);
	...
	spellElement_acidSpray.damage = static_cast<int>(10 * multi);
	...
	spellElement_drainSoul.damage = static_cast<int>(18 * multi);
	...
	spellElement_poison.damage = static_cast<int>(10 * multi);
	...
	spellElementConstructor(&spellElement_compel);
	spellElement_compel.mana = 3;
	spellElement_compel.base_mana = 3;
	spellElement_compel.overload_multiplier = 1;
	spellElement_compel.damage = static_cast<int>(4 * multi);
	spellElement_compel.duration = 0;
	strcpy(spellElement_compel.element_internal_name, "spell_element_compel");
	...
	spellConstructor(&spell_compel);
	strcpy(spell_compel.spell_internal_name, "spell_compel");
	spell_compel.ID = SPELL_COMPEL;
	spell_compel.difficulty = 0;
	node = list_AddNodeLast(&spell_compel.elements);
	node->element = copySpellElement(&spellElement_missile_trio);
	node->size = sizeof(spellElement_t);
	node->deconstructor = &spellElementDeconstructor;
	element = (spellElement_t*)node->element;
	...
	element->elements.last = NULL;
	node = list_AddNodeLast(&element->elements);
	node->element = copySpellElement(&spellElement_compel);
	node->size = sizeof(spellElement_t);
	node->deconstructor = &spellElementDeconstructor;
	element = (spellElement_t*)node->element;
	element->node = node;
