...
int isCharacterValidFromDLC(Stat& myStats, int characterClass)
{
	...
	case CLASS_MESMER:
	case CLASS_BREWER:
	case CLASS_NECROKNIGHT:
	case CLASS_APOTHECARY:
		if (!enabledDLCPack1)
		{
	...
	else if (svFlags & SV_FLAG_MORECLASS || lobbyWindowSvFlags & SV_FLAG_MORECLASS)
	{
		if (characterClass <= CLASS_ASCETIC)
		{
			return VALID_OK_CHARACTER;
		}

		else if (characterClass == CLASS_APOTHECARY || characterClass == CLASS_EXTERMINATOR
			|| characterClass == CLASS_NECROKNIGHT || characterClass == CLASS_LOREMASTER)
		{
			return VALID_OK_CHARACTER;
		}
	...
	{
		if (characterClass >= CLASS_CONJURER && !(characterClass == CLASS_APOTHECARY || characterClass == CLASS_EXTERMINATOR
			|| characterClass == CLASS_NECROKNIGHT || characterClass == CLASS_LOREMASTER))
		{
			if (myStats.playerRace == RACE_HUMAN)
			{
...
static void handleMainMenu(bool mode)
{
	...
	if ( charcreation_step >= 1 && charcreation_step < 6 )
	{
						...
						if ( omousey >= pady && omousey < pady + 17 )
						{
							...
							if ( !disableSelect && inputs.bMouseLeft(clientnum) )
							{
								...
								if ( !disableSelect )
								{
									...
									if ( stats[0]->playerRace != RACE_HUMAN && lastRace != RACE_HUMAN && client_classes[0] > CLASS_ASCETIC
										&& stats[0]->appearance == 0 )
									{
										if ( isCharacterValidFromDLC(*stats[0], client_classes[0]) != VALID_OK_CHARACTER )
										{
											client_classes[0] = CLASS_ASCETIC + stats[0]->playerRace;
										}
										stats[0]->clearStats();
										initClass(0);
									}
									else if ( stats[0]->playerRace != RACE_HUMAN && lastRace == RACE_HUMAN && client_classes[0] > CLASS_ASCETIC
										&& lastAppearance == 0 )
									{
										if ( isCharacterValidFromDLC(*stats[0], client_classes[0]) != VALID_OK_CHARACTER )
										{
						...
						if ( stats[0]->playerRace > 0 )
						{
							if ( omousey < pady + (NUMPLAYABLERACES * 17) + 64 ) // first option
							{
								if ( stats[0]->appearance != 0 )
								{
									...
									// convert human class to monster special classes on reselect.
									if ( client_classes[0] > CLASS_ASCETIC )
									{
										if ( isCharacterValidFromDLC(*stats[0], client_classes[0]) != VALID_OK_CHARACTER )
										{
											client_classes[0] = CLASS_ASCETIC + stats[0]->playerRace;
			...
			if ( keystatus[SDLK_UP] || (inputs.bControllerInputPressed(clientnum, INJOY_DPAD_UP) && rebindaction == -1) )
			{
				...
				else if ( raceSelect == 2 )
				{
					if ( stats[0]->appearance != 0 )
					{
						stats[0]->appearance = 0;
						// convert human class to monster special classes on reselect.
						if ( stats[0]->playerRace != RACE_HUMAN && client_classes[0] > CLASS_ASCETIC )
						{
							if ( isCharacterValidFromDLC(*stats[0], client_classes[0]) != VALID_OK_CHARACTER )
							{
								client_classes[0] = CLASS_ASCETIC + stats[0]->playerRace;
		...
		else if ( charcreation_step == 2 )
		{
			...
			{
				entriesToDisplay = CLASS_ASCETIC + 1;
			}
...
void buttonRandomCharacter(button_t* my)
{
	...
	client_classes[0] = local_rng.rand() % (CLASS_ASCETIC + 1);//NUMCLASSES;
	stats[0]->clearStats();
	if ( enabledDLCPack1 || enabledDLCPack2 )
	{
		...
		if ( stats[0]->playerRace == RACE_HUMAN )
		{
			...
			if ( !enabledDLCPack1 )
			{
				while ( client_classes[0] == CLASS_CONJURER || client_classes[0] == CLASS_ACCURSED
					...
					|| client_classes[0] == CLASS_APOTHECARY || client_classes[0] == CLASS_NECROKNIGHT)
				{
					client_classes[0] = local_rng.rand() % (NUMCLASSES);
				}
			}
			else if ( !enabledDLCPack2 )
			{
				while ( client_classes[0] == CLASS_HUNTER || client_classes[0] == CLASS_SHAMAN
					...
					|| client_classes[0] == CLASS_EXTERMINATOR || client_classes[0] == CLASS_LOREMASTER)
				{
					client_classes[0] = local_rng.rand() % (NUMCLASSES);
				}
			}
		...
		{
			client_classes[0] = local_rng.rand() % (CLASS_ASCETIC + 2);
			if ( client_classes[0] > CLASS_ASCETIC )
			{
				client_classes[0] = CLASS_ASCETIC + stats[0]->playerRace; // monster specific classes.
			}
			stats[0]->appearance = 0;
		}
	}
	...
	if ( lastClass >= 0 && lastSex >= 0 && lastRace >= 0 && lastAppearance >= 0 && lastName != "" )
	{
		...
		switch ( isCharacterValidFromDLC(*stats[index], lastClass) )
		{
			...
			case INVALID_REQUIREDLC2:
				...
				if ( client_classes[index] > CLASS_ASCETIC )
				{
