...
void calculateMean()
{
	int array[MAXPLAYERS];
	int array2[MAXPLAYERS];
	int g;

	for (g = 0; g < MAXPLAYERS; g++)
	{
		if (players[g]->entity != nullptr)
		{
			array[g] = players[g]->entity->getStats()->LVL;
			array2[g] = 1;
		}
		else
		{
	...
	avglvl = static_cast<int>(sum / sum2);
